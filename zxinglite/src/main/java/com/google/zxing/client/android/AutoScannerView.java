...
public class AutoScannerView extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        //循环划线，从上到下
        if (lineOffsetCount > frame.bottom - frame.top - dp2px(100)) {
            lineOffsetCount = 0;
        } else {
...
//            canvas.drawLine(frame.left, frame.top + lineOffsetCount, frame.right, frame.top + lineOffsetCount, linePaint);    //画一条红色的线
            ...
            lineRect.bottom = frame.top + dp2px(100) + lineOffsetCount;
            canvas.drawBitmap(((BitmapDrawable)(getResources().getDrawable(R.drawable.ercode_scan_ray))).getBitmap(), null, lineRect, linePaint);
        }
